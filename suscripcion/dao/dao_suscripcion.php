...
class suscripcion
{
	...
	public function registrar_visita($dispositivo, $navegador, $so, $url, $ip, $capacitacion_id)
	{

		$gbd = Gestionbd::getInstancia();

		$sql = "insert into visitas values(now(),:dispositivo,:navegador,:so,:url,:ip,:capacitacion_id)";

		$cmd = $gbd->prepare($sql);

		$cmd->bindparam(':dispositivo', $dispositivo);

		$cmd->bindparam(':navegador', $navegador);

		$cmd->bindparam(':so', $so);

		$cmd->bindparam(':url', $url);
	...
	public function listado_producto($tipoProducto)
	{
		$gbd = Gestionbd::getInstancia();
		$sql = "SELECT  a.capacitacion_id                 AS id ,UPPER(a.capacitacion_base_nombre) AS nombre FROM ( SELECT  CASE a.capacitacion_base_id WHEN @curType THEN @curRow := @curRow + 1 ELSE @curRow := 1 END AS NUMERO ,@curType := a.capacitacion_base_id                                                          AS capacitacion_base_id ,a.capacitacion_base_nombre ,a.capacitacion_id FROM ( SELECT  c.CAPACITACION_ID ,cb.capacitacion_base_id ,cb.capacitacion_base_nombre ,c.capacitacion_fecha_inicio FROM capacitacion_base cb INNER JOIN capacitacion c ON cb.capacitacion_base_id = c.capacitacion_base_id WHERE DATE(c.capacitacion_fecha_inicio) >= DATE(NOW()) AND NOT c.capacitacion_vacantes = c.capacitacion_matriculados AND c.capacitacion_estado = 1 AND cb.capacitacion_base_tipo IN (:producto) ORDER BY cb.capacitacion_base_id, c.capacitacion_fecha_inicio ) a JOIN ( SELECT  @curRow := 0 ,@curType := '' ) r ) a WHERE a.numero = 1 ORDER BY a.capacitacion_id";
		$cmd = $gbd->prepare($sql);
		$cmd->bindparam(':producto', $tipoProducto);
		$cmd->execute();
		$lista = $cmd->fetchAll(PDO::FETCH_ASSOC);
		return $lista;
	}

	public function listado_productoV2()
	{
		$gbd = Gestionbd::getInstancia();
		$sql = "SELECT  a.capacitacion_id                 AS id ,UPPER(a.capacitacion_base_nombre) AS nombre FROM ( SELECT  CASE a.capacitacion_base_id WHEN @curType THEN @curRow := @curRow + 1 ELSE @curRow := 1 END AS NUMERO ,@curType := a.capacitacion_base_id                                                          AS capacitacion_base_id ,a.capacitacion_base_nombre ,a.capacitacion_id FROM ( SELECT  c.CAPACITACION_ID ,cb.capacitacion_base_id ,cb.capacitacion_base_nombre ,c.capacitacion_fecha_inicio FROM capacitacion_base cb INNER JOIN capacitacion c ON cb.capacitacion_base_id = c.capacitacion_base_id WHERE DATE(c.capacitacion_fecha_inicio) >= DATE(NOW()) AND NOT c.capacitacion_vacantes = c.capacitacion_matriculados AND c.capacitacion_estado = 1 AND c.capacitacion_id IN (3008,3009,3086,3011) ORDER BY cb.capacitacion_base_id, c.capacitacion_fecha_inicio ) a JOIN ( SELECT  @curRow := 0 ,@curType := '' ) r ) a WHERE a.numero = 1 ORDER BY a.capacitacion_id";
		$cmd = $gbd->prepare($sql);
		$cmd->execute();
		$lista = $cmd->fetchAll(PDO::FETCH_ASSOC);
		return $lista;
	...
	public function listado_productoV3()
	{
		$gbd = Gestionbd::getInstancia();
		$sql = "SELECT  a.capacitacion_id                 AS id ,UPPER(a.capacitacion_base_nombre) AS nombre FROM ( SELECT  CASE a.capacitacion_base_id WHEN @curType THEN @curRow := @curRow + 1 ELSE @curRow := 1 END AS NUMERO ,@curType := a.capacitacion_base_id                                                          AS capacitacion_base_id ,a.capacitacion_base_nombre ,a.capacitacion_id FROM ( SELECT  c.CAPACITACION_ID ,cb.capacitacion_base_id ,cb.capacitacion_base_nombre ,c.capacitacion_fecha_inicio FROM capacitacion_base cb INNER JOIN capacitacion c ON cb.capacitacion_base_id = c.capacitacion_base_id WHERE DATE(c.capacitacion_fecha_inicio) >= DATE(NOW()) AND NOT c.capacitacion_vacantes = c.capacitacion_matriculados AND c.capacitacion_estado = 1 AND c.capacitacion_id IN (3012,3086,2975,3010,3035) ORDER BY cb.capacitacion_base_id, c.capacitacion_fecha_inicio ) a JOIN ( SELECT  @curRow := 0 ,@curType := '' ) r ) a WHERE a.numero = 1 ORDER BY a.capacitacion_id";
		$cmd = $gbd->prepare($sql);
		$cmd->execute();
		$lista = $cmd->fetchAll(PDO::FETCH_ASSOC);
		return $lista;
	...
	public function listado_productoV4()
	{
		$gbd = Gestionbd::getInstancia();
		$sql = "SELECT  a.capacitacion_id                 AS id ,UPPER(a.capacitacion_base_nombre) AS nombre FROM ( SELECT  CASE a.capacitacion_base_id WHEN @curType THEN @curRow := @curRow + 1 ELSE @curRow := 1 END AS NUMERO ,@curType := a.capacitacion_base_id                                                          AS capacitacion_base_id ,a.capacitacion_base_nombre ,a.capacitacion_id FROM ( SELECT  c.CAPACITACION_ID ,cb.capacitacion_base_id ,cb.capacitacion_base_nombre ,c.capacitacion_fecha_inicio FROM capacitacion_base cb INNER JOIN capacitacion c ON cb.capacitacion_base_id = c.capacitacion_base_id WHERE DATE(c.capacitacion_fecha_inicio) >= DATE(NOW()) AND NOT c.capacitacion_vacantes = c.capacitacion_matriculados AND c.capacitacion_estado = 1 AND c.capacitacion_id IN (2962,2963,3028,2996) ORDER BY cb.capacitacion_base_id, c.capacitacion_fecha_inicio ) a JOIN ( SELECT  @curRow := 0 ,@curType := '' ) r ) a WHERE a.numero = 1 ORDER BY a.capacitacion_id";
		$cmd = $gbd->prepare($sql);
		$cmd->execute();
		$lista = $cmd->fetchAll(PDO::FETCH_ASSOC);
		return $lista;
	...
	public function getCelularVendedor($usuario){
		$gbd = Gestionbd::getInstancia();
		$sql = "SELECT usuario_celular FROM usuario WHERE usuario_id = :usuario LIMIT 1";
		$cmd = $gbd->prepare($sql);
		$cmd->bindparam(':usuario', $usuario);
		$cmd->execute();
		$lista = $cmd->fetch(PDO::FETCH_ASSOC);
		return $lista['usuario_celular'];
	}

	public function send_mail($from_name, $correo, $nom, $file, $url, $id, $asunto, $mensaje, $correo_adjunto = null)
	{
		$mail = new PHPMailer;
	...
	public function getBenchMark_Email_ID($id)
	{
		...
		if ($response !== false) {
			...
			if (isset($response_data["Response"])) {
				...
				$email["html"] = $email_details["TemplateContent"];

				$email["asunto"] = $email_details["Subject"];
			}
		}
	...
	{
		$gbd = Gestionbd::getInstancia();
		$sql = "insert into ficha_contacto (FICHA_CONTACTO_ID,FICHA_CONTACTO_EMAIL,FICHA_CONTACTO_NOMBRES,FICHA_CONTACTO_APELLIDOS,FICHA_CONTACTO_TELEFONO,FICHA_CONTACTO_MODO,FICHA_CONTACTO_PROFESION,FICHA_CONTACTO_CENTRO_TRABAJO,FICHA_CONTACTO_CARGO,FICHA_CONTACTO_MENSAJE,FECHA_REGISTRO,FICHA_CONTACTO_FLAG_ATENDIDO,FICHA_CONTACTO_FECHA_ATENCION,FICHA_CONTACTO_FLAG_DATOS,FICHA_CONTACTO_HORARIO,FICHA_CONTACTO_USUARIO,FICHA_CONTACTO_NUM_DOCUMENTO,FICHA_CONTACTO_PAIS,FICHA_CONTACTO_PROVINCIA,FICHA_CONTACTO_SEXO,FICHA_CONTACTO_EDAD,FLG_PUBLICIDAD,CAMPANA_ID,FICHA_CONTACTO_PERFIL_OPCIONES) values('',:EMAIL,:NOMBRES,:APELLIDOS,:TELEFONO,:MEDIO,:PROFESION,:CENTRO_TRABAJO,:CARGO,:MENSAJE,NOW(),:FLAG_ATENDIDO,:FECHA_ATENDIDO,:DATOS,NULL,:USUARIO,:DNI,:PAIS,:PROVINCIA,:SEXO,:EDAD,:PUBLICIDAD,:CAMPANA,:PERFIL)";
		$cmd = $gbd->prepare($sql);
		...
		$cmd->bindparam(':USUARIO', $datos['numUsuario']);
		...
		$cmd->bindparam(':PUBLICIDAD', $datos['publicidad']);
		$cmd->bindparam(':CAMPANA', $datos['campana']);
		$cmd->bindparam(':PERFIL', $datos['perfil']);
		$cmd->execute();
		return $gbd->lastInsertId();
	}

	public function registrar_ficha_capacitacion($contacto_id, $capacitacion_id, $capacitacion_base_id, $campanaId = null)
	{
		$gbd = Gestionbd::getInstancia();
			...
			if ($contacto_id > 0) {
				...
				foreach ($cursos as $key => $value) {
					...
					if ($key < 3 || $capacitacion_id == 0) {
						...
						$id = self::registrar_ficha_capacitacion($contacto_id, $capacitacion_id, $capacitacion_base_id,$campanaId);
					}
				}
			}
		}
			...
			if ($data["check_mail"] == 1) {
				...
				if ($status_ok == false) {
					...
					foreach ($dataCapacitacion as $capacitacion) {
						...
						if ($capacitacion["email_id"]>0) {

							$benchmark = self::getBenchMark_Email_ID($capacitacion["email_id"]);

							$titulo = $benchmark["asunto"];

							$mensaje = $benchmark["html"];

						}elseif ($capacitacion["capacitacion_id"] == "2909") {

							$titulo = "Mi primer Dashboard en Looker Studio";

							$mensaje = $mensajedmc->dmcPlayLooker();

						}elseif ($capacitacion["capacitacion_id"] == "1474") {

							$titulo = "Programas de especialización - " . $capacitacion["nombre"];

							$mensaje = $mensajedmc->mensaje_dsf();

						} else if ($capacitacion["capacitacion_id"] == "2681") {

							$titulo = "Conversatorio gratuito: Inteligencia artificial y su impacto en la productividad de las empresas";

							$mensaje = $mensajedmc->mensaje_convervatorio_gratuito();

						}  else if ($capacitacion["capacitacion_id"] == "2724") {

							$titulo = "Curso Gratuito: Técnicas de imputación de datos en Machine Learning";

							$mensaje = $mensajedmc->dmcplayMachineLearning();

						} else if ($capacitacion["capacitacion_id"] == "2580") {

							$titulo = "LIV DATA SUMMIT 2023";

							$mensaje = $mensajedmc->mensaje_summit();
						}else if ($capacitacion["capacitacion_id"] == "3236") {
