...
$datoRadio = "";
$cantidadPreguntas = base64_decode($_GET['pre']);
//$cantidadPreguntas = 5;


  $valorDecodificado = base64_decode($_GET['cap']);
//$valorDecodificado = 70;
...
$resultado = $dao_formularioLogin->consultaEstado($valorDecodificado, $_SESSION['ID']);
 ...
 if (!isset($resultado["intentos"])) { 
  $dao_formularioLogin->agregar_estado_certificaciones($_SESSION['ID'], $valorDecodificado, 1, $datoRadio);
...
//$nombreCap = "Prueba examen pacifico";
...
<form id="form" name="form" method="POST" onsubmit="return enviarFormulario()" action="../../suscripcion/controller/controller_formularioLogin.php?op=GuardarPruebaTecnica" autocomplete="off">
  ...
  <div class="row">
    ...
    <div class="col-md-8">
      ...
      <br><br>
          ...
          <h4 style="color: white">Pregunta <span id="numeroPregunta" >1</span> de <?= $cantidadPreguntas ?> </h4>  <br>
       

      <p style="font-family: Helvetica, Arial, sans-serif; color:white">Tienes <span class="tiempo_estimado"></span> 20 minutos para resolver este Test que consta de <span class="nro_preguntas"></span> preguntas aleatorias. <br> Revisa detenidamente antes de enviar tus respuestas. </p>
      <div id="contador" style="color: #5cc0e9; margin-bottom: 12px; font-size: 1.2em; font-weight: bold;"></div>
    </div>
...
//original
  ...
  // Duración inicial del cronómetro (20 minutos)
  var duracionInicial = 20 * 60; // 1200 segundos

  // var cantidadPreguntas = 20;
  let cantidadPreguntas = <?php echo $cantidadPreguntas ?>;
  ...
  function actualizarContador(num = null) {
    ...
    contadorElemento.innerHTML = "Tiempo restante: " + tiempoFormateado;

    // Si el contador llega a cero, detener el intervalo y enviar automáticamente el formulario
    if (duracion === 0) {
      clearInterval(intervalo);
      contadorElemento.innerHTML = "¡Tiempo finalizado!";

      // Habilitar el botón de envío por si estuviera desactivado y ejecutarlo
      submitButton.disabled = false;
      submitButton.click();
      return; // Evitar que el contador se vuelva negativo
    }
   ...
   // window.location.href = "https://certificaciones.dmc.pe/template/vista/certificaciones-aprobado.php";
    ...
    $.ajax({
      ...
      success: function(data) {
       ...
       // clearInterval(intervalo);
        // Ya no reiniciamos la duración al valor inicial para que el cronómetro continúe.
    ...
    /*    intervalo = setInterval(function() {
        ...
        if (numero == <?php echo $cantidadPreguntas ?>) {
