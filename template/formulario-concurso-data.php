...
if ($resultado["estado_permiso"] > 0) { 
  header('location: ../../suscripcion/inicio_formulario');
 return;
}

 $dat = $dao_formularioLogin->ejecutarFlujoFicha($valorDecodificado, $_SESSION[ID],$_SESSION[CORREO]);


if (!isset($resultado["intentos"])) { 
  $dao_formularioLogin->agregar_estado_certificaciones($_SESSION['ID'],$valorDecodificado,0,null);

}else if ($resultado["intentos"] == 0) {
...
<form id="form" name="form" method="POST" onsubmit="return enviarFormulario()" action="../suscripcion/controller/controller_formularioLogin.php?op=GuardarCconcursoDataScienceAdvanced" autocomplete="off">
  ...
  <div class="row">
    ...
    <div class="col-md-8">
        ...
        <h4 ><b> Pregunta <span id="numeroPregunta" >1</span> de <?= $cantidadPreguntas ?></b></h4>  <br>
     
      <p style="font-family: Helvetica, Arial, sans-serif; color:#8E93A3">Tienes <span class="tiempo_estimado"></span> 40 segundos por pregunta para resolver este examen que consta de <span class="nro_preguntas"></span> preguntas aleatorias. <br> Revisa detenidamente antes de enviar tus respuestas. </p>
    ...
    <div class="form-group div_preguntas col-md-11" id="baseConcurso">
      <div class="encuesta-ct">
        ...
        <div class="pregunta-concurso">
          <label class="pay no-select" style="font-family: Helvetica, Arial, sans-serif;color: #8E93A3;user-select: none">
            <?= $pregunta["pregunta"] ?>
          </label>
          <? foreach (json_decode($pregunta["opciones"], true) as $key => $opc) : ?>
            <div style="font-family: Helvetica, Arial, sans-serif; color:#8E93A3; user-select: none" class="custom-control custom-radio">
...
<script type="text/javascript">
  ...
  var cantidadPreguntas = <?php echo $cantidadPreguntas ?>;
  ...
  function listaPreguntaConcurso() {
    ...
    var capacitacion = <?php echo $valorDecodificado ?>;
    var preguntasId = [];
